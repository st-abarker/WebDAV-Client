...
{
	internal class ReportResponseParser : IResponseParser<ReportResponse>
	{
		private readonly LockResponseParser _lockResponseParser;

		public ReportResponseParser([DisallowNull] LockResponseParser lockResponseParser)
		{
		...
		public ReportResponse Parse(string response, int statusCode, string description)
		{
			if (string.IsNullOrEmpty(response))
				return new ReportResponse(statusCode, description);
			...
			var resources = xresponse.Root.LocalNameElements("response", StringComparison.OrdinalIgnoreCase)
				...
				.ToList();
			return new ReportResponse(statusCode, description, resources);
		}
